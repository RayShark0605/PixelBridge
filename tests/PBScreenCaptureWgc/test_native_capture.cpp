...
class ScreenOracle final : public RoiConsumer
{
public:
    ...
    CaptureStatus Submit(const RoiFrameMetadata& metadata, ID3D11Texture2D* texture, ID3D11DeviceContext* context) override
    {
        ...
        // Readback exists only in this oracle, not in the production backend or FrameArrived.
...
struct Fixture
{
    ...
    WgcCaptureConfig captureConfig;
    std::uint16_t whiteHalf = 0x3c00;
};

void PrintSnapshot(const WgcCaptureSnapshot& snapshot)
{
...
TEST_CASE("WGC native monitor capture verifies actual screen pixels, Recreate and cleanup")
{
    Fixture fixture;
    for (const bool forceQuery : {false, true})
    {
        ...
        const auto status = WgcCaptureTestAccess::Create(fixture.captureConfig, consumer, MakeNativeCaptureBackend(options), capture);
        ...
        REQUIRE(beforeStop.frameLeaseHighWater <= fixture.captureConfig.queuedFrameLimit + fixture.captureConfig.roiTextureCount + 1);
...
TEST_CASE("WGC native initialization faults and asynchronous timeout cleanup are reversible")
{
    Fixture fixture;
    for (const auto stage : {CaptureStage::Adapter, CaptureStage::Device, CaptureStage::CaptureItem, CaptureStage::TextureRing,
                            CaptureStage::FramePool, CaptureStage::Session})
    {
        NativeCaptureOptions options;
        options.failAfterStage = stage;
        std::cout << "inject native initialization failure after stage=" << static_cast<int>(stage) << std::endl;
        std::unique_ptr<WgcCapture> capture;
        const auto status = WgcCaptureTestAccess::Create(fixture.captureConfig, std::make_shared<ScreenOracle>(), MakeNativeCaptureBackend(options), capture);
    ...
    std::unique_ptr<WgcCapture> capture;
    REQUIRE(WgcCaptureTestAccess::Create(config, std::make_shared<ScreenOracle>(), MakeNativeCaptureBackend(options), capture));
    fixture.Present(2);
    REQUIRE(WaitFor([&]
    {
