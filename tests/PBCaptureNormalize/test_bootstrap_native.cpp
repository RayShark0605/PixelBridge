...
struct NativeFixture
{
    void Initialize(const Clock::time_point deadline, const bool desktopLevels = false)
    {
        ...
        const POINT requestedOrigin{PB_TEST_MONITOR_ORIGIN_X, PB_TEST_MONITOR_ORIGIN_Y};
        const HMONITOR selectedMonitor = MonitorFromPoint(requestedOrigin, MONITOR_DEFAULTTONULL);
        REQUIRE(selectedMonitor != nullptr);
        MONITORINFO monitor{};
        monitor.cbSize = sizeof(monitor);
        REQUIRE(GetMonitorInfoW(selectedMonitor, &monitor));
        const auto left = static_cast<std::int64_t>(requestedOrigin.x);
        const auto top = static_cast<std::int64_t>(requestedOrigin.y);
        INFO("Native Bootstrap requires the explicitly configured physical test origin and never falls back to the primary output");
        REQUIRE(left >= monitor.rcMonitor.left);
        REQUIRE(top >= monitor.rcMonitor.top);
        REQUIRE(left + width <= monitor.rcMonitor.right);
        REQUIRE(top + height <= monitor.rcMonitor.bottom);
