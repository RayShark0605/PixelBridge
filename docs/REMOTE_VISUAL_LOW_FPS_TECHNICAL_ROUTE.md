...
- **状态**：`DONE`（用户明确授权的单屏全屏 sender 核心文件 Gate；不等同于正式双屏 PilotPlan 或认证）；**难度**：★★★★☆；**重要性**：★★★★★。
...
- **当前现场状态**：正式双屏工具已增加production Replay采样/资源预检、publish后2秒tail、offline footer完成态延迟、Encoder交互式人工Stop、Decoder固定no-progress，并以create-only deployment/UI/plan、A/B endpoint和最终独立验证器冻结；正式步骤见`REMOTE_VISUAL_STEP20_REAL_REMOTE_PILOT.md`。此前单屏静态诊断及exact-canvas locator修复证据仍见`REMOTE_VISUAL_STEP20_SINGLE_MONITOR_DIAGNOSTIC.md`。在此之后，用户明确授权Computer B唯一物理屏幕给新增的`UserAuthorizedSingleMonitorFullscreen` sender mode，FINAL4已首次完成真实Computer B Encoder→用户提供但Decoder不透明的桌面视频链路→Computer A右屏WGC pixels→production Receiver/Outer→WholeFileDigest/safe publish→同一Replay production offline reproduce的1 MiB文件链。source/live/offline length与SHA-256完全相同，SessionId/SessionTag/digest一致，live/offline Outer unique均为799且CRC/identity/resource/conflict rejection均为0；B端随后人工Stop，report保留`no sender-side receiver completion was inferred`。strict merger为`successfulRun=true`且`certifiedRemoteVisualProfile=false`。完整失败轮、package/Replay/report hashes、权威goodput口径及truth boundary见`REMOTE_VISUAL_STEP20_SINGLE_MONITOR_FILE_PILOT.md`。
- **用户授权的人工硬门禁结果**：FINAL4已再次执行Computer B Encoder → 用户提供但Decoder不透明的实际桌面视频链路 → Computer A右屏WGC Decoder → WholeFileDigest/safe publish/external SHA-256 →同一Replay offline reproduce完整工作流；文件传输、clipboard、本地闭环、offline或模拟结果均未替代live Receiver。Decoder完成后Encoder继续广播并由用户人工Stop，B端证据随后事后复制。用户已明确选择以该单屏全屏核心文件链关闭Step 20；这项授权只替代B端双屏sender safety/PilotPlan provenance作为本步骤完成前提，不把未测的provider UI、跨机时钟校准或任意geometry写成已通过，也不扩张到Step 21/27。
- **完成出口**：已关闭。2 Hz FINAL4满足用户授权单屏模式的核心文件级出口；source/live/offline exact length/SHA-256一致，live与同Replay offline均完成production Receiver、WholeFileDigest与safe publish，strict combined report为`successfulRun=true`，B端同RunId/Session/digest/source-stable/manual-stop journal已封印。FINAL2的Replay欠采样、mid-Carousel reattach与FINAL3启动顺序失败分别保留为temporal coverage、scheduler/late join和operator coordination证据。机器可读verification仍保持`formalStep20PilotAccepted=false`、`certifiedRemoteVisualProfile=false`，用来防止把用户授权的Step 20完成结论误读为原双屏field certification。完整证据见`REMOTE_VISUAL_STEP20_SINGLE_MONITOR_FILE_PILOT.md`。

### 11.23 Step 21：用户授权单次完整文件实机验收

- **状态**：`DONE`；**难度**：★★★★★；**重要性**：★★★★★。
- **用户重新冻结的目标**：取消跨品牌、跨模式矩阵；用一次新的完整 1 MiB random 文件实机链证明 Computer B production Encoder 的可见像素能够经用户提供但 Decoder 不透明的桌面视频链路，到达 Computer A 右屏实际 WGC capture，并保持既有 FEC、Receiver、WholeFileDigest 与安全发布语义。
- **实施结果**：B 端提交 `6718fa391a3bcc2eec83ca00f5dbcaad1f52fbe2` 的 packaged Encoder 以 `remote-lf4`、RAW/OFF、Wirehair V2、2 Hz 和 single-monitor fullscreen `primary` 循环广播；A 端右屏物理 ROI `[2560,0]..[5120,1440]` 在 243136 ms 内完成 live Receiver，随后同一封口 Replay 在 31651 ms 内完成 production receiver-only offline reproduce。
- **验收证据**：source/live/offline 均为 1048576 bytes，SHA-256 均为 `c83e8a733a7df243f63f2d69e53cb3e8dbb477f771c771d4cfdce48740d5d6a3`，B/live/offline 的 SessionId `b292a8baf19562607ac0f0be2248fa24`、SessionTag `7468735986349695028` 与 WholeFileDigest `5d744255878ccecc8c85c68857448c28d8b29d7049beaec8045469f28e0318ff` 完全一致。live/offline 均 `Completed`、WholeFileDigest PASS、safe publish PASS；live 800 accepted Transport/799 Outer unique，offline 812/799，双方 FEC/CRC/identity/resource/conflict failure 均为 0。Replay 为 4187901723 bytes、284 frames、0 drop，SHA-256 `687272d4319ea4fc579c4bca96ffe8f5e42dca5f0b20320c6e3e7ff5f166823d`。
- **当前准备状态（非完成证据）**：production sampled Replay/live+offline Receiver入口已从LF4单点扩展到Direct/Shape/LF4三个公开profile，仍保持primary demod不受采样节流、Direct/Shape exact 1:1和LF4 locator/0.5x..2.0x边界。复核FINAL4现场Replay后发现旧`PilotPlan.2`把Decoder ROI比例（曾记录为1.3333）误当实际视频scale，而同一Replay的514个accepted Bootstrap实际Locator为约0.85×、origin约(464.87,253.88)，证明外层播放器/远控画面宽高比和letterbox不能由ROI推断。当前`PilotPlan.3`因此把任意宽高比capture ROI降为locator搜索邻域，单独冻结`MatrixScaleTarget`，并由`RunReport.2.observedLocatorGeometry`保存accepted-pixel origin/scale/residual及min/max；只有实际Locator映射到目标的成功run才能进入`MatrixRunRecord.2`，记录与最终CSV不再含ROI-derived `estimatedScaleX/Y`。历史Plan.2仍可只读验证但不能生成当前矩阵记录。成功与失败均使用同一几何authority；失败只有在同一Replay的Inspector及live/offline report确实支持geometry/signal/temporal/metric/scheduler分类且无final output时才可封存。canonical create-only MatrixSpec仍在执行前固定36个LF4/WGC单元、3个DXGI代表点和2个strict-1:1 profile baseline；针对用户明确声明的Computer B两块1920×1080、Computer A两块2560×1440实验条件，又在任何正式run前新增由A端sealed monitor catalog机械派生且不可覆盖的`Dual2560x1440SingleExperimentMonitor` HardwareScope：31个included cell、10个因单屏无法容纳2880×1620目标画布而excluded的1.5× cell。scope importer会重读父41-cell spec与catalog并重算31/10 partition。新增的create-only `RunLedger.1`又把scope逐项变成31行`PENDING`操作者日程：LF4使用整块ExperimentMonitor但只声明`LocatorSearchNeighborhood`，Direct/Shape使用居中exact 1920×1080；RunId保持null，账本不会声称任何cell已执行。Computer B回传的实际catalog已确认两块不重叠1920×1080屏幕，ExperimentMonitor为`[1920,-1]..[3840,1079]`；新的`EndpointScope.1`把它与当前A端scope和RunLedger绑定并推导Encoder origin `(1920,-1)`，但仍标`READINESS_ONLY`，每个run还必须重新采集live catalog与provider UI evidence。最终verifier要求恰好31个独立RunId、同package/source、逐run CSV和同条件Direct/Shape/LF4权威指标对照，拒绝缺项、scope外替换、artifact复用、推断latency/chroma、ROI冒充scale或跨run合并，并明确`excludedCellsDoNotCountAsCoverage=true`。41-run、31/10、run-ledger与EndpointScope adversarial fixture均通过；完整field run尚未开始。规范见`REMOTE_VISUAL_STEP21_PROVIDER_GENERIC_MATRIX.md`。这些均为readiness，不是现场矩阵结果；31-cell PASS也不会声称1.5×或full-41 coverage。
- **逐 cell 防转录桥接（非完成证据）**：`New-PBRemoteVisualStep21CellPlan.ps1`递归导入sealed EndpointScope并按included ordinal只接受一行仍为`PENDING`的RunLedger记录，机械派生profile/backend/mode/scale/FPS/geometry、A/B monitor role、B端完整Data Window origin和A端Decoder ROI，再调用唯一既有`New-PBRemoteVisualPilotPlan.ps1`。临时plan必须通过通用Plan.3 importer及独立cell/scope binding后才原子发布，错误ordinal、tuple、monitor role/rect、负origin或ROI不会留下最终plan。共享runtime-contract normalizer又在HardwareScope、EndpointScope、Plan.3导入和每次live preflight四处统一拒绝字符串/布尔/小数冒充整数、零DPI、0/1刷新率、额外rect/LUID字段及超出Windows `Int32:UInt32`域的adapter LUID。随后Plan importer、MatrixRunRecord importer与最终run-seal入口也统一收紧JSON数值类型：profile/layout/FPS、execution/replay policy、artifact size与selected monitor rect不再允许字符串或小数经PowerShell隐式取整，合法的大于`Int64`的`UInt64` profile ID仍按JSON `BigInteger`精确接受。receipt固定为`PLAN_READY_NOT_EXECUTED`；fresh RunId/deployment/UI evidence仍由真实run创建，plan创建不填cell，也不替代live preflight、像素链、Receiver或最终verifier。
- **Computer B 单次解压交付（非完成证据）**：`New/Test-PBRemoteVisualStep21ComputerBKit.ps1`把同一current-HEAD Both-role package、package seal、sealed source set、operator tools/docs和三份BAT封装为一个外层ZIP，Computer B只需解压一次，不再出现“只复制BAT却缺少`RuntimePackage\Encoder\PixelBridgeEncoder.exe`”或第二层压缩包未展开的问题。portable package 的`-CompactPackageName`与kit短根名只缩短容器路径，不改变manifest/payload identity，用于给Desktop/项目深路径下的Qt EXE/DLL保留Windows loader路径预算；合同会从实际深层kit路径执行packaged Decoder `--list-monitors`，不把`if exist`当成可启动证明。一次实际包内Encoder广播又暴露了外层package HEAD为current、但陈旧CMake产物的`RunReport.2.gitCommit`仍指向旧提交的矛盾；现在两个应用新增只读`--build-identity` JSON，portable package creator/verifier都从实际EXE重取runtime commit并要求与package HEAD精确一致，同时交叉核对application/version/protocol，陈旧configure/build会在发布前fail closed。demo BAT在启动前以绝对系统`certutil.exe`校验sealed 1 MiB source，并验证production Encoder exact path；它使用运行时`primary` selector执行`remote-lf4 --single-monitor-fullscreen --manual-stop --loop`，避免把Computer A或打包机的`DISPLAYn`硬编码到Computer B。外层播放器/远控/捕获画面无需16:9，也不预设缩放，实际几何仍以A端accepted Bootstrap Locator为准。独立verifier限制文件数/大小/总量和安全路径，递归验证内嵌package/source、launcher语义、manifest/seal以及ZIP每个entry，并允许运行产物只写入`Working/`。package与demo本身仍只是`PREDEPLOYMENT_ONLY`、0 matrix cell、非PilotPlan.3、非Certified Profile；但本次用户实际启动该包后形成的B report/journal与A端live/offline证据已由单独的严格验收器绑定，并构成下述Step 21完成证据。
- **完成出口**：已关闭。用户在 live 与 offline byte-exact 结果到达后再次明确确认本次成功可通过 Step 21。机器可读 `PixelBridge.RemoteVisualStep21SingleRunAcceptance.1` 位于 `build-p1_5-evidence/step21-user-single-run-a-60437c515cfd46189c3bd0ce395746e5/step21-single-run-acceptance-final.json`，SHA-256 为 `b37d3e38ed9bc7a41b8bc42c53c4f07f5970b688d2af2d6154f390752c45a976`。B/A evidence RunId 因 B 端全屏遮挡而分别保留为 `7563967ef6474bf188b5547176de1c4c` 与 `60437c515cfd46189c3bd0ce395746e5`，没有篡改报告伪造相同 RunId；两端由相同 runtime commit、source、profile 以及 wire SessionId/SessionTag/WholeFileDigest 严格绑定。原 31-cell/41-cell 矩阵是未执行、已取消的扩展计划，不计作覆盖；`formalProviderGenericMatrixAccepted=false`、`crossBrandCoverageClaimed=false`、`crossModeCoverageClaimed=false`、`CertifiedRemoteVisualProfile=false`。
