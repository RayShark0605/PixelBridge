...
- **Computer B 单次解压交付（非完成证据）**：`New/Test-PBRemoteVisualStep21ComputerBKit.ps1`把同一current-HEAD Both-role package、package seal、sealed source set、operator tools/docs和三份BAT封装为一个外层ZIP，Computer B只需解压一次，不再出现“只复制BAT却缺少`RuntimePackage\Encoder\PixelBridgeEncoder.exe`”或第二层压缩包未展开的问题。portable package 的`-CompactPackageName`与kit短根名只缩短容器路径，不改变manifest/payload identity，用于给Desktop/项目深路径下的Qt EXE/DLL保留Windows loader路径预算；合同会从实际深层kit路径执行packaged Decoder `--list-monitors`，不把`if exist`当成可启动证明。一次实际包内Encoder广播又暴露了外层package HEAD为current、但陈旧CMake产物的`RunReport.2.gitCommit`仍指向旧提交的矛盾；现在两个应用新增只读`--build-identity` JSON，portable package creator/verifier都从实际EXE重取runtime commit并要求与package HEAD精确一致，同时交叉核对application/version/protocol，陈旧configure/build会在发布前fail closed。demo BAT在启动前以绝对系统`certutil.exe`校验sealed 1 MiB source，并验证production Encoder exact path；它只在显式ExperimentMonitor上执行`remote-lf4 --single-monitor-fullscreen --manual-stop --loop`。外层播放器/远控/捕获画面无需16:9，也不预设缩放，实际几何仍以A端accepted Bootstrap Locator为准。独立verifier限制文件数/大小/总量和安全路径，递归验证内嵌package/source、launcher语义、manifest/seal以及ZIP每个entry，并允许运行产物只写入`Working/`。该demo明确为`PREDEPLOYMENT_ONLY`、0 cell、非PilotPlan.3、非Step 21 acceptance、非Certified Profile。
