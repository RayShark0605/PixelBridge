...
**退出标准：** mandatory corpus CPU/GPU accepted-byte parity；false accepted/conflict 为 0；Base neutralized >=16 KiB。未达 32 KiB 可继续首版，但必须明确记录为 engineering target miss。
**验证结果（2026-09-04）：** Release 重新配置并增量构建 `PBDemodD3D11Tests`、`PBUnifiedVisualCpuTests`、`PBUnifiedTransformCorpusTests`、`PBCaptureNormalizeTests` 与 `PBCapturePipelineTests` 成功。新增 G12 独立注册 Gate `PBUnifiedVisualGpuParityTests` 1/1（2 cases / 32,979 assertions，164.75 秒）通过：复用 G10 同一份 18-case mandatory transform 定义，在 WARP、`AMD Radeon(TM) Graphics` 与 `NVIDIA GeForce RTX 5090 D` 各运行一次，形成 54 条 adapter/scenario 记录；每个 adapter 有 15 条合法 Bootstrap/identity 场景进入 GPU，crop、Bootstrap copy conflict 与 caller identity conflict 三条在 GPU 前按产品 admission 边界零输出。进入 GPU 的每条记录均逐 slot 比较 lane/kind、FEC disposition、padding、CRC、Session/Frame identity、accepted bytes 与 Base/Fine/Chroma erasure；accepted block 集合逐字节相等，未比较逐 bit 浮点 soft metric，三个 backend 的 false accepted、独立 truth mismatch 与 conflict output 均为 0，且 `rawPixelReadbackBytes=0`、device/shutdown 状态正常。另在 WARP 用 clean mixed Control/Transport 和 5 个独立损坏 slot 覆盖 Control CRC、非规范 padding、Transport CRC、错误 SessionTag 与 Inner-FEC failure；损坏帧只接受预期的 26/31 blocks。性能 Gate 使用 256 KiB OS CSPRNG、RAW、正式 Session/Manifest/Segment Control、Wirehair V2、ReceiverIngress 和 PBStorage：`0.75x + neutral chroma` 夹具只让 Base Luma slot 引入新 equation，Fine/Chroma 仅携带同帧 Base equation 的逐字节一致重复，最终以 200 个唯一 Base、0 个唯一非 Base equation 在 13 个 unique logical frames 后恢复、校验、发布并重新打开，`262,144 / 13 = 20,164.923 B/frame`，超过 16 KiB 硬门槛；clean 全 lane 在 7 帧完成，`262,144 / 7 = 37,449.143 B/frame`，同时超过 32 KiB 工程目标。两个指标都只在 Segment 验证、whole-file BLAKE3、`.part` 安全发布、final reopen 逐字节及摘要复核完成后才置为 available。共享 transform 定义提取后，G10 Golden corpus 1/1（9,042,053 assertions）继续逐字节一致；`PBUnifiedVisualCpuTests` 10/10（1,022,588 assertions）与 `PBCaptureNormalizeTests [normalize-contract]` 12/12（1,305 assertions）通过。额外扩展执行的历史 LF4 `PBCapturePipelineTests [capture-pipeline][warp]` 不属于 G12 Unified exit gate，其中 3/4 cases 通过，旧 LF4 WGC/DXGI 两个 section 均在同一首帧 60 秒 age/await 边界超时；隔离重跑可复现，未改动的 LF4 单帧 GPU 邻接测试本机约 25 秒。该额外结果未被伪装为通过，也未在 G12 中通过放宽 age gate 或超时预算顺手修改旧 Profile。
**退出：** 已满足。mandatory corpus 在三个要求 backend 上关闭最终 accepted-byte parity，WARP 的 Control/Transport 语义损坏覆盖了 FEC、padding、CRC 与 identity disposition；全套 G12 记录的 false accepted、truth mismatch 与 conflict output 均为 0。Base-only-unique `0.75x + neutral chroma` 最终发布指标为 `20,164.923 B/unique logical frame`，超过 `16,384` 硬门槛；clean 指标为 `37,449.143 B/unique logical frame`，因此本次没有 32 KiB engineering target miss。结论仅覆盖 synthetic/headless WARP 与当前可枚举 D3D11 adapter，不构成真实远控 provider、真实 capture 或实屏认证。
**产物：** G10/G12 共享 mandatory case 定义位于 `tests/PBModulation/unified_transform_corpus_cases.h`，CPU corpus 复用位于 `tests/PBModulation/test_unified_transform_corpus.cpp`，G12 parity/协议损坏/最终发布性能 Gate 位于 `tests/PBDemodD3D11/test_g12_semantic_parity.cpp` 并由 `tests/PBDemodD3D11/CMakeLists.txt` 注册。本地机器可读证据位于 `build-unified-release/tests/PBDemodD3D11/g12-unified-gpu-parity.actual.jsonl`、`g12-unified-publish-metrics.actual.jsonl` 与 `build-unified-release/Testing/Temporary/LastTest.log`；定向 CPU/CaptureNormalize 日志位于 `build-unified-release/tests/PBModulation/g12-unified-cpu.txt`、`build-unified-release/tests/PBCaptureNormalize/g12-normalize-contract.txt`，额外 LF4 超时证据位于同目录的 `g12-capture-pipeline-warp.txt` 与 `g12-capture-pipeline-lf4-isolated.txt`。
