...
// PB-Interleave-RemoteVisual-X2 is a distinct physical mapping. It must not be
// returned by GetDesktopLevelsPermutation: the Phase-1 Direct-Level mappings
// and their inverse remain frozen at 2x2/4x4.
struct RemoteVisualPermutation
{
    std::uint32_t tilePixels;
    std::uint32_t tileCount;
    std::uint32_t rowTiles;
    ...
    [[nodiscard]] constexpr bool IsValid() const noexcept
    {
        return tilePixels == 8 && tileCount == 16723 && rowTiles == 104;
    }

    [[nodiscard]] constexpr std::uint32_t ToPhysical(const std::uint32_t logical, const std::uint64_t sequence) const noexcept
    {
        return !IsValid() || logical >= tileCount ? tileCount :
            static_cast<std::uint32_t>(detail::ApplyAffineTile(logical, tileCount, 65537, (sequence % 16) * rowTiles));
    }
    ...
    [[nodiscard]] constexpr std::uint32_t ToLogical(const std::uint32_t physical, const std::uint64_t sequence) const noexcept
    {
        ...
        const std::uint64_t unshifted = (physical + static_cast<std::uint64_t>(tileCount) - (sequence % 16) * rowTiles) % tileCount;
        return static_cast<std::uint32_t>(detail::ApplyAffineTile(unshifted, tileCount, 10330, 0));
    }
};
...
inline constexpr RemoteVisualPermutation kRemoteVisualPermutation{8, 16723, 104};
static_assert((65537ull * 29825) % 346752 == 1 && (65537ull * 29825) % 86688 == 1);
static_assert((65537ull * 10330) % 16723 == 1);
static_assert(15 * 864 < 346752 && 15 * 432 < 86688);
static_assert(15 * 104 < 16723);
